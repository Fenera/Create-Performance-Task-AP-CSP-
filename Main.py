...
# -ory of the image)
#Code one line below is a modified version from https://www.geeksforgeeks.org/python-os-path-abspath-method-with-example/
...
#Initialize Fonts for later use using built-in font class
...
#They correlate to the questions in the quest.ans dictionary keys
...
#
#parameters -> pygame_gui.elements.UITextEntryLine(rectangle, (length, height)), manager = what ever your manager is set to, object_id = "#name of the entry field(can be anything)")
...
#If user got a correct answer, the a check mark will be shown with no transparency(255) and then become transparent(0)
def correct_img(x, y):
    #transparency off
...
#If user got a incorrect answer, the an 'x' mark will be shown with no transparency(255) and then become transparent(0)
...
def check_correct(question, response):
    #quest_ans[question].lower() is what the answer is(ex: 'a', 'b', 'c')
...
while run:
    ...
    #Turquoise colored heading. Parameters = pygame.draw.rect(screen, [color as rgb], [left, top, length, height], filled)
    ...
    #I got the idea of using a frame rate from https://pythonprogramming.altervista.org/pygame-how-to-display-the-frame-rate-fps-on-the-screen/
    clock.tick(FPS)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame_gui.UI_TEXT_ENTRY_FINISHED and event.ui_object_id == "#text_box_entry":
            ...
            #If the variable question_index is less than the list of questions, then the game continues(more questions available)      
